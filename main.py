...
biz_owner_cache = {}     # {business_connection_id: owner_user_id} — Secretary Mode
...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS owner_pause (
        ...
        last_owner_ts TEXT,
        PRIMARY KEY (business_connection_id, contact_chat_id)
    );
    ''')
...
def set_owner_pause(bcid, contact_id):
    con = db()
    con.execute("""INSERT INTO owner_pause (business_connection_id, contact_chat_id, last_owner_ts)
                   VALUES (?,?,?)
                   ON CONFLICT(business_connection_id, contact_chat_id)
                   DO UPDATE SET last_owner_ts=excluded.last_owner_ts""",
                (bcid, contact_id, _now()))
    con.commit(); con.close()

def is_owner_paused(bcid, contact_id, hours=2):
    con = db(); c = con.cursor()
    c.execute("SELECT last_owner_ts FROM owner_pause WHERE business_connection_id=? AND contact_chat_id=?",
              (bcid, contact_id))
    row = c.fetchone(); con.close()
    if not row: return False
    last = datetime.fromisoformat(row['last_owner_ts'])
...
SECRETARY_SYS = (
    "তুমি Shohan-এর ব্যক্তিগত Telegram একাউন্টে আসা মেসেজের রিপ্লাই দিচ্ছ, যখন ও ব্যস্ত বা "
    "অনলাইনে নেই। খুব স্বাভাবিক, বন্ধুত্বপূর্ণ, ক্যাজুয়াল ভাষায় কথা বলবে — অফিসিয়াল বা "
    "রোবোটিক টোন একদম না, নিজে থেকে বারবার 'আমি AI/প্রোগ্রাম' এসব বলে পরিচয় দিতে যাবে না। "
    "তবে কেউ যদি সরাসরি জিজ্ঞেস করে তুমি Shohan কিনা, বা তুমি AI/বট কিনা — তাহলে সত্যি কথা "
    "বলবে: তুমি Shohan-এর AI assistant, ওর হয়ে সাময়িকভাবে মেসেজ দেখছ। এটা কখনো অস্বীকার "
    "করবে না বা মিথ্যা বলবে না।"
)

def ai_chat(uid, text, system_prompt=None):
    ...
    try:
        resp = groq_client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[{"role":"system","content":system_prompt or CHAT_SYS}]+chat_history[uid],
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not await check_access(u.id, ctx.bot, update.effective_message.reply_text): return
...
async def cmd_admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id not in ADMIN_IDS:
        await update.effective_message.reply_text("❌ তুমি admin নও।"); return
    s = get_stats()
    await update.effective_message.reply_text(
        f"👑 *Admin Panel*\n\n"
...
async def handle_file(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if state.get('action')=='timing_wait_file' and fname.endswith(('.srt','.vtt','.ass','.ssa')):
        ...
        await update.effective_message.reply_text("✅ ফাইল পেয়েছি!\n\nকত সেকেন্ড shift? (যেমন: +5 বা -3.5)",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ বাতিল", callback_data="home")]])); return
    ...
    if state.get('action')=='merge_wait_first' and fname.endswith(('.srt','.vtt','.ass','.ssa')):
        ...
        await update.effective_message.reply_text("✅ প্রথম ফাইল পেয়েছি!\n\n*দ্বিতীয়* ফাইলটা পাঠাও।",
            parse_mode='Markdown',
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ বাতিল", callback_data="home")]])); return

    # Merge second
    if state.get('action')=='merge_wait_second' and fname.endswith(('.srt','.vtt','.ass','.ssa')):
        msg = await update.effective_message.reply_text("⏳ Merge হচ্ছে...")
        try:
    ...
    if any(fname.endswith(ext) for ext in AUDIO_EXTS):
        if doc.file_size and doc.file_size > MAX_AUDIO:
            await update.effective_message.reply_text("❌ ২৫MB-এর বেশি!"); return
    ...
    if u.id in active_tasks and not active_tasks[u.id]:
        await update.effective_message.reply_text("⚠️ একটি অনুবাদ চলছে! আগেরটা শেষ করো।"); return
    ...
    if not content: await update.effective_message.reply_text("❌ ফাইল পড়া যাচ্ছে না!"); return
    blocks = parse_auto(content, doc.file_name)
    if not blocks: await update.effective_message.reply_text("❌ subtitle নেই!"); return

    total = len(blocks); cost = calc_srt_cost(total); bal = get_tokens(u.id)
    if bal < cost:
        await update.effective_message.reply_text(token_warn(cost,bal), parse_mode='Markdown', reply_markup=kb_back()); return
    ...
    status = await update.effective_message.reply_photo(
        photo=pie_chart(0,1),
    ...
    try:
        ...
        except: await update.effective_message.reply_text(notice, parse_mode='Markdown', reply_markup=kb)
    finally:
...
# ══════════════════════════════════════════════
# 🤖  SECRETARY MODE (Business Messages)
# ══════════════════════════════════════════════
async def get_biz_owner_id(bcid, bot):
    if bcid in biz_owner_cache:
        return biz_owner_cache[bcid]
    try:
        conn = await bot.get_business_connection(bcid)
        biz_owner_cache[bcid] = conn.user.id
        return conn.user.id
    except Exception as e:
        logger.warning(f"get_business_connection failed: {e}")
        return None

async def handle_business_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    msg = update.effective_message
    if not msg or not msg.text: return
    bcid = msg.business_connection_id
    if not bcid: return

    owner_id   = await get_biz_owner_id(bcid, ctx.bot)
    contact_id = msg.chat.id
    sender_id  = msg.from_user.id if msg.from_user else None
    if not sender_id: return

    # owner (আমি) নিজে এই কন্টাক্টকে মেসেজ দিলে — ২ ঘণ্টার জন্য AI থামিয়ে দাও
    if owner_id and sender_id == owner_id:
        set_owner_pause(bcid, contact_id)
        return

    # pause চলাকালীন কন্টাক্ট মেসেজ দিলে চুপ থাকো
    if is_owner_paused(bcid, contact_id):
        return

    loop  = asyncio.get_event_loop()
    reply = await loop.run_in_executor(
        executor, functools.partial(ai_chat, f"biz_{sender_id}", msg.text, SECRETARY_SYS))
    try:
        await ctx.bot.send_message(chat_id=contact_id, text=reply, business_connection_id=bcid)
    except Exception as e:
        logger.warning(f"business reply failed: {e}")
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if state.get('action') == 'img_wait_topic':
        topic = text.strip()
        if not topic:
            await update.effective_message.reply_text("❌ বিষয়টা লেখো!"); return
    ...
    if state.get('action') == 'img_wait_count':
        try:
            ...
            await update.effective_message.reply_text("❌ শুধু সংখ্যা লেখো! (যেমন: 10)"); return
    ...
    if state.get('action') == 'timing_wait_offset':
        ...
        except: await update.effective_message.reply_text("❌ সংখ্যা লেখো! (+5 বা -3.5)"); return
        msg = await update.effective_message.reply_text("⏳ Timing ঠিক করছি...")
        try:
    ...
    if state.get('action') == 'admin_broadcast' and u.id in ADMIN_IDS:
        uids = all_uids(); msg = await update.effective_message.reply_text(f"📢 পাঠাচ্ছি {len(uids)} জনকে...")
        ok = fail = 0
        for uid2 in uids:
        ...
        if state.get('action')==act and u.id in ADMIN_IDS:
            try:
                ...
                await update.effective_message.reply_text(
                    ...
                    parse_mode='Markdown', reply_markup=kb_admin())
            except: await update.effective_message.reply_text("❌ ভুল ID")
            user_state.pop(u.id, None); return
    ...
    if state.get('action') == 'admin_lookup' and u.id in ADMIN_IDS:
        try:
            ...
            else: await update.effective_message.reply_text("❌ পাওয়া যায়নি।")
        except: await update.effective_message.reply_text("❌ ভুল ID")
        user_state.pop(u.id, None); return

    if state.get('action') == 'admin_tokens_uid' and u.id in ADMIN_IDS:
        try: user_state[u.id] = {'action':'admin_tokens_amount','target':int(text)}; await update.effective_message.reply_text("কত token?")
        except: await update.effective_message.reply_text("❌ ভুল ID")
        return
    ...
    if state.get('action') == 'admin_tokens_amount' and u.id in ADMIN_IDS:
        try:
            ...
            await update.effective_message.reply_text(f"✅ `{t}` কে *{n} tokens* দেওয়া হয়েছে।",
                parse_mode='Markdown', reply_markup=kb_admin())
        except: await update.effective_message.reply_text("❌ ভুল")
        user_state.pop(u.id, None); return

    # ── YouTube Subtitle ──
    if state.get('action') == 'yt_wait_url':
        if not ('youtube.com' in text or 'youtu.be' in text):
            await update.effective_message.reply_text("❌ সঠিক YouTube link দাও!"); return
        cost = COST['youtube']; bal = get_tokens(u.id)
        if bal < cost:
            user_state.pop(u.id, None)
            await update.effective_message.reply_text(token_warn(cost,bal), parse_mode='Markdown', reply_markup=kb_back()); return
        deduct_tokens(u.id, cost); user_state.pop(u.id, None)
        msg = await update.effective_message.reply_text("▶️ *YouTube subtitle download হচ্ছে...*\n⏳", parse_mode='Markdown')
        loop = asyncio.get_event_loop()
    ...
    if ctx.user_data.get('awaiting_search'):
        ...
        try:
            if poster_data:
                await msg.delete()
                await update.effective_message.reply_photo(photo=io.BytesIO(poster_data),
    ...
    if chat_mode.get(u.id, False):
        ...
        if bal < cost:
            await update.effective_message.reply_text(token_warn(cost,bal),
                parse_mode='Markdown', reply_markup=kb_back()); return
        deduct_tokens(u.id, cost)
...
def main():
    ...
    # ── Business messages (Secretary Mode: personal account auto-reply) ──
